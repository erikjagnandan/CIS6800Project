...
model_string: set this to 'cnn' or 'cnn_regularized', depending on the desired model (it will be automatically loaded from the models folder)

batch_size: we set this to 10 for training our CNN adapter, which was performed on an NVIDIA 4090 GPU with ~25 GB of RAM - this may need to set this lower to avoid CUDA out of memory errors, depending on your hardware

batches_per_backprop: we implemented gradient accumulation - this sets the number of batches to accumulate gradients for before performing a gradient update - we set this to 1 for training our CNN adapter
